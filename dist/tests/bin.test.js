"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
var bin_1 = require("../bin");
var index = require("../");
describe('bin', function () {
    beforeEach(function () {
        jest.restoreAllMocks();
    });
    test('warns on missing endpoint', function () { return __awaiter(_this, void 0, void 0, function () {
        var consoleWarnMock, consoleLogMock, getHeadersFromInputMock, getRemoteSchemaMock, printToFileMock;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    consoleWarnMock = jest
                        .spyOn(console, 'warn')
                        .mockImplementation(function () { return false; });
                    consoleLogMock = jest
                        .spyOn(console, 'log')
                        .mockImplementation(function () { return false; });
                    getHeadersFromInputMock = jest.spyOn(index, 'getHeadersFromInput');
                    getRemoteSchemaMock = jest.spyOn(index, 'getRemoteSchema');
                    printToFileMock = jest.spyOn(index, 'printToFile');
                    /**
                     * Execution
                     */
                    return [4 /*yield*/, bin_1.main({ input: [] })
                        /**
                         * Tests
                         */
                    ];
                case 1:
                    /**
                     * Execution
                     */
                    _a.sent();
                    /**
                     * Tests
                     */
                    expect(consoleWarnMock).toBeCalledWith('No endpoint provided');
                    expect(consoleWarnMock).toBeCalledTimes(1);
                    expect(consoleLogMock).toBeCalledTimes(0);
                    expect(getHeadersFromInputMock).toBeCalledTimes(0);
                    expect(getRemoteSchemaMock).toBeCalledTimes(0);
                    expect(printToFileMock).toBeCalledTimes(0);
                    return [2 /*return*/];
            }
        });
    }); });
    test('warns on fetch error', function () { return __awaiter(_this, void 0, void 0, function () {
        var consoleWarnMock, consoleLogMock, getHeadersFromInputMock, getRemoteSchemaMock, printToFileMock;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    consoleWarnMock = jest
                        .spyOn(console, 'warn')
                        .mockImplementation(function () { return false; });
                    consoleLogMock = jest
                        .spyOn(console, 'log')
                        .mockImplementation(function () { return false; });
                    getHeadersFromInputMock = jest.spyOn(index, 'getHeadersFromInput');
                    getRemoteSchemaMock = jest
                        .spyOn(index, 'getRemoteSchema')
                        .mockResolvedValue({ status: 'err', message: 'pass' });
                    printToFileMock = jest.spyOn(index, 'printToFile');
                    /**
                     * Execution
                     */
                    return [4 /*yield*/, bin_1.main({
                            input: ['pass'],
                            flags: { method: 'POST', json: false },
                        })
                        /**
                         * Tests
                         */
                    ];
                case 1:
                    /**
                     * Execution
                     */
                    _a.sent();
                    /**
                     * Tests
                     */
                    expect(consoleWarnMock).toBeCalledWith('pass');
                    expect(consoleWarnMock).toBeCalledTimes(1);
                    expect(consoleLogMock).toBeCalledTimes(0);
                    expect(getHeadersFromInputMock).toBeCalledTimes(1);
                    expect(getRemoteSchemaMock).toBeCalledTimes(1);
                    expect(printToFileMock).toBeCalledTimes(0);
                    return [2 /*return*/];
            }
        });
    }); });
    test('logs obtained schema when no output', function () { return __awaiter(_this, void 0, void 0, function () {
        var consoleWarnMock, consoleLogMock, getHeadersFromInputMock, getRemoteSchemaMock, printToFileMock;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    consoleWarnMock = jest
                        .spyOn(console, 'warn')
                        .mockImplementation(function () { return false; });
                    consoleLogMock = jest
                        .spyOn(console, 'log')
                        .mockImplementation(function () { return false; });
                    getHeadersFromInputMock = jest.spyOn(index, 'getHeadersFromInput');
                    getRemoteSchemaMock = jest
                        .spyOn(index, 'getRemoteSchema')
                        .mockResolvedValue({ status: 'ok', schema: 'pass' });
                    printToFileMock = jest.spyOn(index, 'printToFile');
                    /**
                     * Execution
                     */
                    return [4 /*yield*/, bin_1.main({
                            input: ['pass'],
                            flags: { method: 'POST', json: false },
                        })
                        /**
                         * Tests
                         */
                    ];
                case 1:
                    /**
                     * Execution
                     */
                    _a.sent();
                    /**
                     * Tests
                     */
                    expect(consoleLogMock).toBeCalledWith('pass');
                    expect(consoleWarnMock).toBeCalledTimes(0);
                    expect(consoleLogMock).toBeCalledTimes(1);
                    expect(getHeadersFromInputMock).toBeCalledTimes(1);
                    expect(getRemoteSchemaMock).toBeCalledTimes(1);
                    expect(printToFileMock).toBeCalledTimes(0);
                    return [2 /*return*/];
            }
        });
    }); });
    test('prints obtained schema when output is defined', function () { return __awaiter(_this, void 0, void 0, function () {
        var consoleWarnMock, consoleLogMock, getHeadersFromInputMock, getRemoteSchemaMock, printToFileMock;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    consoleWarnMock = jest
                        .spyOn(console, 'warn')
                        .mockImplementation(function () { return false; });
                    consoleLogMock = jest
                        .spyOn(console, 'log')
                        .mockImplementation(function () { return false; });
                    getHeadersFromInputMock = jest.spyOn(index, 'getHeadersFromInput');
                    getRemoteSchemaMock = jest
                        .spyOn(index, 'getRemoteSchema')
                        .mockResolvedValue({ status: 'ok', schema: 'pass' });
                    printToFileMock = jest
                        .spyOn(index, 'printToFile')
                        .mockImplementation(function () { return false; });
                    /**
                     * Execution
                     */
                    return [4 /*yield*/, bin_1.main({
                            input: ['pass'],
                            flags: { method: 'POST', json: false, output: 'path' },
                        })
                        /**
                         * Tests
                         */
                    ];
                case 1:
                    /**
                     * Execution
                     */
                    _a.sent();
                    /**
                     * Tests
                     */
                    expect(consoleWarnMock).toBeCalledTimes(0);
                    expect(consoleLogMock).toBeCalledTimes(0);
                    expect(getHeadersFromInputMock).toBeCalledTimes(1);
                    expect(getRemoteSchemaMock).toBeCalledTimes(1);
                    expect(printToFileMock).toBeCalledTimes(1);
                    return [2 /*return*/];
            }
        });
    }); });
    test('correctly reduces headers', function () { return __awaiter(_this, void 0, void 0, function () {
        var consoleWarnMock, consoleLogMock, getHeadersFromInputMock, getRemoteSchemaMock, printToFileMock;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    consoleWarnMock = jest
                        .spyOn(console, 'warn')
                        .mockImplementation(function () { return false; });
                    consoleLogMock = jest
                        .spyOn(console, 'log')
                        .mockImplementation(function () { return false; });
                    getHeadersFromInputMock = jest.spyOn(index, 'getHeadersFromInput');
                    getRemoteSchemaMock = jest
                        .spyOn(index, 'getRemoteSchema')
                        .mockResolvedValue({ status: 'ok', schema: 'pass' });
                    printToFileMock = jest
                        .spyOn(index, 'printToFile')
                        .mockImplementation(function () { return false; });
                    /**
                     * Execution
                     */
                    return [4 /*yield*/, bin_1.main({
                            input: ['pass'],
                            flags: {
                                method: 'POST',
                                json: false,
                                output: 'path',
                                header: ['key=value'],
                            },
                        })
                        /**
                         * Tests
                         */
                    ];
                case 1:
                    /**
                     * Execution
                     */
                    _a.sent();
                    /**
                     * Tests
                     */
                    expect(consoleWarnMock).toBeCalledTimes(0);
                    expect(consoleLogMock).toBeCalledTimes(0);
                    expect(getHeadersFromInputMock).toBeCalledTimes(1);
                    expect(getRemoteSchemaMock).toBeCalledTimes(1);
                    expect(getRemoteSchemaMock).toBeCalledWith('pass', {
                        method: 'POST',
                        json: false,
                        headers: {
                            'Content-Type': 'application/json',
                            key: 'value',
                        },
                    });
                    expect(printToFileMock).toBeCalledTimes(1);
                    return [2 /*return*/];
            }
        });
    }); });
});
//# sourceMappingURL=bin.test.js.map